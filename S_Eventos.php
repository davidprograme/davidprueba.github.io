...
<head>
    ...
    <title>NOVA | Sección De Servicios </title>
...
<section class="section_opciones_usuario">
    ...
    <?php
        require_once "bdd.php";
         $sql = "SELECT * FROM usuario_nuevo_nova WHERE checkbox = 'Servicios Eventos'";
         $resultado = mysqli_query($con, $sql);

         if(mysqli_num_rows($resultado) > 0){
            while($fila = mysqli_fetch_assoc($resultado)){

                echo '<!--- carta presentacion de los cliente de NOVA --->
...
';
            ...
            echo "<div class='div_else'>Lo siento, no hay empresas en esta categoría.</div>";
         }
         mysqli_close($con2);
...
.buscar:focus, .input_buscar:focus{
    width: 300px;
    outline: none;
    border-radius: 15px;
    border-top-color: gray;
...
/*--------------------- CLINTES CUENTA PUBLICIDAD ---------------------------*/
.section_publiclientes{
    margin: 50px auto;
    display: flex;
}
.div_publiclientes{
...
/*---PUBLICIDAD DE LA PAGINA (SLIDER-IMAGENES) ---*/
.div_img{
    width: 300px;
    padding: 5px;
    margin: -10px 10px;
...
/*---- DIV INFORMACION DE LA EMPRESA ---*/
.div_datos{
    margin: -300px 280px;
    width: 750px;
    border: 1px solid transparent;
}
.ul_datos_empresa_cliente{
    list-style: none;
    color: black;
    font-family: 'Montserrat', sans-serif;
}
